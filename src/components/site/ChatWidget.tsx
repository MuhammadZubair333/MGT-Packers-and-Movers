...
const QUOTE_KEYWORDS =
  /shift|mov|reloc|hous|home|offic|cargo|transport|book|quot|price|cost|rate|how much|servic|pack|load|stor|carrier|truck|rent|deliver|international|shipment/i;

function userWantsService(messages: Message[], assistantIndex: number): boolean {
  const prev = messages[assistantIndex - 1];
  return !!(prev && prev.role === "user" && QUOTE_KEYWORDS.test(prev.content));
}
                  ...
                  <div className="flex max-w-[80%] flex-col gap-2">
                    ...
                    {userWantsService(messages, i) && (
                      <button
                        ...
                        className="flex items-center gap-3 rounded-xl bg-navy-deep px-3.5 py-2.5 shadow-[0_4px_14px_rgba(10,25,60,0.35)] transition-all hover:-translate-y-0.5 hover:brightness-110"
                      >
                        <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/15">
                          <FileText className="h-4 w-4 text-white" />
                        </div>
                        <div className="flex flex-col text-left">
                          <span className="text-[11px] font-medium text-white/70">Get a price in minutes</span>
                          <span className="text-xs font-bold text-white">Fill Contact Form →</span>
                        </div>
                      </button>
                    )}
                    <a
                    ...
                    >
                      <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/20">
                        <svg className="h-[18px] w-[18px] shrink-0" viewBox="0 0 24 24" fill="white">
